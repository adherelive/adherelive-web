...
const logger = createLogger("MOBILE > APPOINTMENT > HELPER");
...
export const getFavoriteInDetails = async (
  ...
  try {
    ...
    logger.error("mgetFavouriteInDetails error", error);
    return null;
  }
};
...
const medicalTestFavorites = async (userTypeData, types) => {
  try {
    ...
    logger.error("m-medicalTestFavorites error", error);
  }
};
...
const radiologyTypeFavorites = async (userTypeData, types) => {
  try {
    ...
    logger.error("m-radiologyTypeFavorites error", error);
  }
};
