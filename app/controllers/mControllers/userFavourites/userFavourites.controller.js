...
class UserFavouritesController extends Controller {
    constructor() {
     ...
     create = async (req, res) => {
      ...
      try {
        const {
          body,
          userDetails: {userId, userCategoryId ,  userData: { category = "" } = {} } = {}
        } = req;
        ...
        Log.debug("userFavourite create 500 error", error);
    ...
    removeFavourite = async (req, res) => {
      ...
      try {
        ...
        const existing = await UserFavouritesService.findExistingFavourite({id});
        if(existing){
