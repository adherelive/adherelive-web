...
const staticTranslations = require('../../../routes/api/prescriptions/translations.json');
...
class PdfService {
    ...
    async translateText(text, targetLang) {
        if (staticTranslations[ text ] && targetLang === 'hi') return staticTranslations[ text ];
        ...
        return new Promise((resolve, reject) => {
            redisClient.get(cacheKey, async (err, cached) => {
                ...
                try {
                    ...
                    redisClient.setex(cacheKey, 3600, translated); // Cache for 1 hour
                    logger.debug('Text translated successfully!')
                    resolve(translated);
                } catch (error) {
                    logger.debug('Error translating text:', error);
                    reject(error);
                }
            });
        });
