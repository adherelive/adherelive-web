...
const addMedicineSchema = Joi.object().keys({
  name: Joi.string().required().label("Please enter valid medicine name"),
  type: Joi.string().required().label("Please select valid medicine type"),
  generic_name: Joi.string().optional().allow("", null),
});
