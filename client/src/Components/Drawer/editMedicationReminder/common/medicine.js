...
class Medicine extends Component {
    ...
    setDefaultMedicine = () => {
        ...
        const {basic_info: {name: default_name = '', id: default_id = null} = {}} = medicines[medId] || {};
                ...
                <Option key={`opt-${med_id}-new-med`} value={med_id}>
                    ...
                    >
                        <Button
                            ...
                            onClick={this.handleAddMedicineOpen}>{`${this.formatMessage(messages.addMedicine)} `}<span
                            className="fw800">{` "${inputText}"`}</span></Button>
                    </div>
                </Option>
            )
        }
    ...
    getFavouriteOptions = () => {
        ...
        const {searching = false, medicine_name: med_name = '', medicine_id: med_id = null} = this.state;
        ...
        if (med_id !== null) { //display selected option with favourites
            options.push(
                <Option key={`opt-${med_id}`} value={med_id}
                        className="pointer flex wp100  align-center justify-space-between"
                    // onMouseDown={this.handleOnMouseDownPreventDef}
    ...
    getOptions = () => {
        const {hits = {}, inputText = '', searching = false, defaultHits = {}} = this.state;
        const {favourite_medicine_ids = []} = this.props;
