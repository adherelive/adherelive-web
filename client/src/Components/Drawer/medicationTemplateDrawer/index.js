    ...
    return (
        ...
        >
            <div className='template-name-modal-container'>
                <div
                    className='template-name-modal-text'>{(carePlanTemplateId && (carePlanName !== BLANK_TEMPLATE)) ? formatMessage(messages.youModified) : formatMessage(messages.youCreated)}</div>
                <Input
                ...
                <div className='template-name-modal-name-button' onClick={saveTemplate}>
                    <div className='template-name-modal-name-text'>{formatMessage(messages.saveTemplate)} </div>
                </div>
                <div className='template-name-modal-skip-button' onClick={skip}>
                    <div className='template-name-modal-skip-text'>{formatMessage(messages.skip)} </div>
                </div>
            </div>
...
class TemplateDrawer extends Component {
    ...
    componentDidMount() {
        ...
        if (carePlanTemplateId) {
            ...
            for (let mId of template_medication_ids) {
                let newMedication = {};
                let {
                    basic_info: {id = 0, care_plan_template_id = 0, medicine_id = 0} = {},
                    schedule_data = {}
            ...
            for (let vId of template_vital_ids) {
                let newVital = {};
                const {
                    basic_info: {care_plan_template_id = 0, id = 0, vital_template_id = 0} = {},
                    details: {
            ...
            for (let wId of template_workout_ids) {
                const {
                    basic_info: {care_plan_template_id = 0, id = 0, name = ''} = {},
                    total_calories = null, duration = null, time = '',
                    details: {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (prevcarePlanTemplateId !== carePlanTemplateId) {
            const {
                ...
                template_vitals = {}, template_diets = {}, template_workouts = {}, medicines
            } = this.props;
    ...
    renderEditMedicationForm = () => {
        ...
        return (
            ...
            <div className='form-block-ap'>
                <div
                    className='form-headings-ap flex align-center justify-start'>{this.formatMessage(messages.medicine)}</div>
                <Input
                ...
                <div
                    className='form-headings-ap mt18 flex align-center justify-start'>{this.formatMessage(messages.frequency)}</div>
                <Input
    ...
    renderEditAppointmentForm = () => {
        ...
        return (
            ...
            <div className='form-block-ap'>
                <div
                    className='form-headings-ap mt18 flex align-center justify-start'>{this.formatMessage(messages.reason)}</div>
                <Input
                    placeholder="Frequency"
                    value={reason}
                    className={"form-inputs-ap"}
                ...
                <div
                    className='form-headings-ap mt18 flex align-center justify-start'>{this.formatMessage(messages.notes)}</div>
    ...
    renderTemplateDetails = () => {
        const {
            ...
            carePlanTemplateIds = [], carePlanTemplateId
        } = this.state;
        ...
        return (
            <div className='template-block'>
                {Object.keys(carePlanTemplateIds).length && showDropDown ? (
                    <Select value={carePlanTemplateId} className={'template-drawer-select wp100'}
                            onChange={this.setTemplateId}>{this.getCarePlanTemplateOptions()}</Select>) : null}
                <div className='wp100 flex align-center justify-space-between'>
                    <div className='form-category-headings-ap '>{this.formatMessage(messages.medications)}</div>
                    <div className='add-more'
                         onClick={this.showAddMedication}>{this.formatMessage(messages.addMore)}</div>
                        ...
                        <div className='flex wp100 flex-grow-1 align-center' key={key}>
                            <div className='drawer-block'>
                                <div className='flex direction-row justify-space-between align-center'>
                                    ...
                                    <Icon type="edit" className='ml20' style={{color: '#4a90e2'}} theme="filled"
                                          onClick={this.showInnerForm(EVENT_TYPE.MEDICATION_REMINDER, key)}/>
                ...
                <div className='wp100 flex align-center justify-space-between'>
                    <div
                        className='form-category-headings-ap align-self-start'>{this.formatMessage(messages.appointments)}</div>
                    <div className='add-more'
                         onClick={this.showAddAppointment}>{this.formatMessage(messages.addMore)}</div>
                </div>
                {
                    appointmentKeys.map(key => {
                        ...
                        const {
                            reason = '',
                            schedule_data: {description = '', date = '', start_time = ''} = {},
                            time_gap = ''
                        } = appointments[key];
                        ...
                        return (
                            ...
                            <div className='flex wp100 flex-grow-1 align-center' key={key}>
                                <div className='drawer-block'>
                                    ...
                                    <div
                                        className='drawer-block-description'>{date ? `After ${moment(date).diff(moment(), 'days')} days` : time_gap ? `After ${time_gap - 1} days` : ''}</div>
                ...
                <div className='wp100 flex align-center justify-space-between'>
                    <div
                        className='form-category-headings-ap align-self-start'>{this.formatMessage(messages.actions)}</div>
                ...
                <div className='wp100 flex align-center justify-space-between'>
                    <div
                        className='form-category-headings-ap align-self-start'>{this.formatMessage(messages.diets)}</div>
                        ...
                        return (
                            ...
                            <div className='flex wp100 flex-grow-1 align-center' key={key}>
                                <div className='drawer-block'>
                                    ...
                                    <div
                                        className='drawer-block-description'>{`${total_calories ? total_calories : '--'}${" "}Cal`}</div>
                ...
                <div className='wp100 flex align-center justify-space-between'>
                    <div
                        className='form-category-headings-ap align-self-start'>{this.formatMessage(messages.workouts)}</div>
    ...
    validateData = (medicationsData, appointmentsData, vitalData, dietData, workoutData) => {

        for (let medication of medicationsData) {
            let {
                medicine = "", medicineType = "", medicine_id = "",
                schedule_data: {
                    quantity = 0, repeat = "", repeat_days = [], start_date = moment(),
                    start_time = moment(), strength = 0, unit = "", when_to_take = [], when_to_take_abbr = ''
                } = {}
            } = medication;
        ...
        for (let appointment of appointmentsData) {
            let {
                reason = '', schedule_data: {
                    date = '',
                    end_time = '', start_time = '', treatment_id = ''
                } = {}
            } = appointment;
        ...
        for (let diet of dietData) {
            ...
            if (!start_date || repeat_days.length === 0 || !name || Object.keys(diet_food_groups).length === 0) {
        ...
        for (let workout of workoutData) {
            ...
            if (!time || !start_date || repeat_days.length === 0 || !name || Object.keys(workout_exercise_groups).length === 0) {
    ...
    onPreSubmit = () => {
        ...
        let templateDataExists = (Object.values(medications).length || Object.values(appointments).length) || Object.values(vitals).length || Object.values(diets).length || Object.values(workouts).length ? true : false;
    ...
    onSubmit = () => {
        ...
        for (let appointment in appointmentsData) {
            ...
            let {
                reason = '', schedule_data: {
                    date = '',
                    end_time = '', start_time = '', treatment_id = '', type = '', appointment_type = ''
                } = {}, time_gap = ''
            } = newAppointment;
    ...
    addWorkout = (data) => {
        ...
        console.log("21381237123127318371283712  ===>>>>> ", {e_date, start_date});
        ...
        return (
            <Fragment>
                ...
                >
                    ...
                    <EditMedicationReminder medicationData={medicationData} medicationVisible={showInner}
                                            editMedication={this.editMedication} hideMedication={this.onCloseInner}
                                            deleteMedicationOfTemplate={this.deleteEntry}/>}
                    {innerFormKey && innerFormType == EVENT_TYPE.APPOINTMENT &&
                    <EditAppointmentDrawer appointmentData={appointmentData} appointmentVisible={showInner}
                    ...
                    <EditVitalDrawer vitalData={vitalData} vitalVisible={showInner} editVital={this.editVital}
                                     hideVital={this.onCloseInner} deleteVitalOfTemplate={this.deleteEntry}/>}
                    {innerFormKey && innerFormType == EVENT_TYPE.DIET &&
                    <EditDietDrawer dietData={dietData} dietVisible={showInner} editTemplateDiet={this.editDiet}
                                    hideDiet={this.onCloseInner} deleteDietOfTemplate={this.deleteEntry}
                                    isDietVisible={isDietVisible}/>}
                    {innerFormKey && innerFormType == EVENT_TYPE.WORKOUT &&
                    <EditWorkoutDrawer workoutData={workoutData} workoutVisible={showInner}
                                       editTemplateWorkout={this.editWorkout} hideWorkout={this.onCloseInner}
                                       deleteWorkoutOfTemplate={this.deleteEntry} isWorkoutVisible={isWorkoutVisible}/>}


                    {showAddMedicationInner && <EditMedicationReminder medicationVisible={showAddMedicationInner}
                                                                       addMedication={this.addMedication}
                                                                       hideMedication={this.closeAddMedication}/>}
                    {showAddAppointmentInner && <EditAppointmentDrawer appointmentVisible={showAddAppointmentInner}
                                                                       addAppointment={this.addAppointment}
                                                                       hideAppointment={this.closeAddAppointment}
                                                                       patientId={patientId} patients={patients}
                                                                       carePlan={carePlan}/>}

                    {showAddVitalInner && <EditVitalDrawer vitalVisible={showAddVitalInner} addVital={this.addVital}
                                                           hideVital={this.closeAddVital}/>}
                    {showAddDietInner && <EditDietDrawer dietVisible={showAddDietInner} addTemplateDiet={this.addDiet}
                                                         hideDiet={this.closeAddDiet} isDietVisible={isDietVisible}/>}
                    {showAddWorkoutInner &&
                    <EditWorkoutDrawer workoutVisible={showAddWorkoutInner} addTemplateWorkout={this.addWorkout}
                                       hideWorkout={this.closeAddWorkout} isWorkoutVisible={isWorkoutVisible}/>}
                ...
                <TemplateNameModal visible={this.state.showTemplateNameModal} hideModal={this.hideNameModal}
                                   carePlanTemplateId={carePlanTemplateId} carePlanName={carePlanName}
                                   changeTemplateName={this.setTemplateName} saveTemplate={this.submitWithName}
                                   skip={this.submitWithOutName} formatMessage={this.formatMessage}/>
            </Fragment>
        );
    }
}
