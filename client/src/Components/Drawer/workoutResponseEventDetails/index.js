...
class WorkoutReponseEventDetailsDrawer extends Component {
    ...
    getScheduleEventData =  async () => {
        try{
        const { schedule_event_id = null , getWorkoutScheduleEventDetails}=this.props;
    ...
    getDetailsComponent = (data) => {
        const {name = '',response = {},notes ='',content_id = null }=data || {};
        const {exercise_contents = {},repetitions={} } = this.props;
        let src = '',content_type="none";
        if(content_id){
            const {video :{content='',content_type:video_content_type =VIDEO_TYPES.NONE} = {}}=exercise_contents[content_id] || {};
            src=content;
         ...
         return (
             <div className="flex direction-column wp100 b-light-grey p10 br5 mt20" >
                 <div className=" fs16 fw800 flex " >
                    ...
                    <div className={`${src.length > 0 ? "ml10" : "null" } flex direction-column align-center justify-center`} >
                        {name}
                    </div>
    ...
    getWorkoutData = () => {
        ...
        for(let each in workout_responses){
            ...
            const { basic_info:{ exercise_detail_id = null, }={}, details : { notes = '' } = {} } = exercise_groups[exercise_group_id] || {};
            const { basic_info:{ exercise_id = null } = {} } = exercise_details[exercise_detail_id] || {};
            const { basic_info:{ name = ''} = {} } = exercises[exercise_id] || {};
            for(let group of workout_exercise_groups){
                const { exercise_group_id : grp_id = null , exercise_content_id = null } = group || {};
            ...
            allDetails.push(this.getDetailsComponent({name,response,notes,content_id}));
        ...
        return (
            ...
            >
                
                {
                    ...
                    ?
                        <video controls  src={videoToShow} alt="exercise content" className="wp100" />
                    :
                    ...
                    ?
                        ...
                        :
                        <video controls  src={newsrc} alt="exercise content" className="wp100" />
    ...
    openModal = ({src,content_type}) => () => {

        this.setState({ videoToShow: src , content_type }, () => this.setState({ videoModalVisible: true }));
    }
    ...
    render() {                                                                                                                                                                                                                                                                                                             
        const {
            visible,
            intl: {formatMessage} = {},
            workout_name = '' , date : props_date = null
        } = this.props;
        const {onClose} = this;
