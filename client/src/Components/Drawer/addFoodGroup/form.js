...
class AddFoodGroupForm extends Component {
  ...
  async componentDidUpdate(prevProps){
    ...
    const { visibleAddFoodDrawer : prev_visibleAddFoodDrawer = false } = prevProps;
  ...
  getPortionOptions = () => {
    ...
    let options = [] , portion_ids = [] , canEdit = true;
    ...
    for(let each in searched_food_item_details ){ 
        ...
        const {
         ...
         if( state_food_item_id == food_item_id ){ // matching portions
  ...
  handleExistingPortionSelect = ({detail_id:value,editable}) => () => {
    ...
    handleDifferentPortionSelect = ({editable}) => () => { // non existing details for item
  ...
  setFoodItemId = (value) =>  {
    ...
    if( prev_food_item_id !== value && Object.keys(searched_food_item_details).length){
      ...
      for(let each in searched_food_item_details ){
        const {basic_info : { food_item_id , creator_id = null , creator_type = null } = {} } = searched_food_item_details[each] || {};
