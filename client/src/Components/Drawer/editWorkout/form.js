...
class WorkoutFieldsFrom extends Component {
  ...
  componentDidMount(){
    ...
    if(editTemplateWorkout){
      const {details:{repeat_days : template_r_days=[]}={}}=workoutData || {};
  ...
  handleClickOutside = (event) =>  {
    const timekeeperDiv = document.getElementsByClassName("react-timekeeper")[0];
    if(timekeeperDiv){
  ...
  getTimePicker = () => {
    ...
    return (
      <TimeKeeper
        time={
          moment(state_time).format("hh:mm A")
        }
  ...
  render() {
    ...
    if(editTemplateWorkout !== null){
      ...
      const {name : template_name  = '', total_calories :template_total_calories = 0 ,
      start_date:temp_start_date=null,
      end_date : temp_end_date = null,
      duration=null,details:{ not_to_do :template_not_to_do = '' }={}}=workoutData || {};
                    ...
                    <div className="mb20 select-days-form-content" >
                      <div className="flex justify-space-between">
                        <div className="flex" >
                          ...
                          <span className="fs14">{this.formatMessage(messages.dayOftheWeek)}</span>
                        </div>
                        <div
                         className=
