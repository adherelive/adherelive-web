...
class SymptomsDrawer extends Component {
    ...
    renderBody = () => {
        ...
        console.log('4895724839uf89324uufu489urfj', this.props.payload.data, body_part, side, description);
        return (
            <div className='humanbody-container hp100'>
                ...
                {description && description.length ? (<div className='fs16 tal'>{`${description}`}</div>) : null}
                {imageUrl && imageUrl.length ? (<Fragment>
                    {imageUrl.length > 0 ? (
                        <div
