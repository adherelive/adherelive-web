...
class EditExerciseGroupForm extends Component {
  constructor(props) {
    ...
    this.handleExerciseSearch = debounce(this.handleExerciseSearch.bind(this), 200); 
  ...
  async componentDidMount() {
    ...
    for(let each in exercise_contents){
      const {basic_info : { exercise_id : ex_id = null } ={} ,video : { content_type = null , content = ''  } = {} } = exercise_contents[each] || {};
  ...
  async componentDidUpdate(prevProps){
    ...
    const {basic_info : { exercise_id : previous_exercise_id = null } = {} } = searched_exercise_details[exercise_detail_id] || {};
    const {basic_info : {name = ''} = {} } = searched_exercises[previous_exercise_id] || {};


    const {
      exercise_detail_id : prev_exercise_detail_id = null,
      visibleAddExerciseDrawer : prev_visibleAddExerciseDrawer = false  } = prevProps;

    if(!visibleAddExerciseDrawer && prev_visibleAddExerciseDrawer !== visibleAddExerciseDrawer){
      if(created){  // fill new created exercise's details
  ...
  fillAllNewCreatedExerciseDetails = () => {
      ...
      const { latest_created_exercise : { exercises = {} , exercise_details = {} , exercise_contents = {} } = {} } = this.props;
      const { setExerciseName , setEditable , setExerciseDetailId } = this.props;
      let exercise_id = null , detail_id = null;
    ...
    setExerciseDetailId(detail_id);
      ...
      calorific_value = null } = exercise_details[detail_id] || {};
    ...
    if(Object.keys(exercise_contents).length){
      ...
      const {video : { content_type = null , content = ''  } = {} } = exercise_contents[key] || {};
  ...
  checkIfDoctorCanEdit = () => {
    ...
    const {
      exercise_details,
      exercise_detail_id,
      authenticated_category}=this.props;
      ...
      if(creator_type === authenticated_category && creator_id && doctor_id && creator_id.toString() === doctor_id.toString() ){
        canEdit=true;
      }else{
        canEdit=false;
      }
  ...
  getRepeatTypeOptions = () => {
    ...
    const { exercise_id : state_exercise_id = null , doctor_id = null }  = this.state;
    const { 
      searched_exercise_details ,
      repetitions = {} ,
    ...
    let options = []  , repetition_ids = [], canEdit = true;

    const { exercise_detail_ids = [] } = searched_exercises[state_exercise_id] || {};
    ...
    for( let each of exercise_detail_ids ){ 
        ...
        const {
            ...
            creator_id=null,
            creator_type=null } = detail;

         if(creator_type === authenticated_category && 
          ...
          options.push(
              <Option
                key={`${each}-${type}`} value={repetition_id}
                onClick={this.handleExistingPortionSelect({detail_id,editable:canEdit})}
              >
                {type}
              </Option>
          )
  ...
  handleExistingPortionSelect = ({detail_id:value,editable}) => () => {
      ...
      const {
       ...
       } = {} , calorific_value = 0 } =  searched_exercise_details[value] || {};
    ...
    handleDifferentPortionSelect = ({editable}) => () => { // non existing details for exercise
  ...
  getExerciseOptions = () => {
    ...
    for(let each in searched_exercises ){
        ...
        const { basic_info : { name : exercise_name ,  id : exercise_id } = {} } = exercise || {};
  ...
  setExerciseId = (value) =>  {
    ...
    const { doctor_id =null , exercise_id : prev_exercise_id = null } = this.state;

    const { basic_info : { name = '' } = {} , exercise_detail_ids = []  } = searched_exercises[value] || {};
    ...
    if( prev_exercise_id !== value ){
      ...
      const {basic_info : { exercise_id  } = {} , creator_id = null , creator_type = null } = searched_exercise_details[first] || {};
      ...
      const {
          ...
          calorific_value = 0  } =  searched_exercise_details[first] || {};


      let flag=false;
      for(let each in exercise_contents){
        const {basic_info : { exercise_id = null } ={} ,video : { content_type = null , content = ''  } = {} } = exercise_contents[each] || {};
  ...
  onBlur = (props) =>{
    ...
    const {
      form: { setFieldsValue , getFieldValue  } = {}}=this.props;
      if( isNotANumber ){
  ...
  render() {
    const {
      ...
      editExerciseGroupDetails , exercise_details,
      editable=false
    } = this.props;
    ...
    const {
      ...
      notes = null } = editExerciseGroupDetails || {};
