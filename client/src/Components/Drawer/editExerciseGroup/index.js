...
class EditExerciseGroup extends Component {
  ...
  handleSubmit = (e) => {
    ...
    const { exercise_detail_id = null,editable = false , exercise_name = ''  } = this.state;
    const { uploadedVideoUrl = '' , videoContentType : video_content_type = null } = this.state;

    const { detail_id  : prev_detail_id = null ,exercise_group_index = null , similar_index = null , exercise_group_id = null , sets : prev_sets = 1 } = editExerciseGroupDetails || {};
    ...
    validateFields(async (err, values) => {
      if (!err) {
        ...
        const { calorific_value :prev_calVal = 0 } = exercise_details[prev_detail_id] || {};
        ...
        let editedExerciseGroupData = {
          ...
          exercise_detail_id: exercise_detail_id ? parseInt(exercise_detail_id) : null,
          notes,
          prev_calorific_val,
          prev_sets,
        ...
        const updateData={
          ...
          try {
            ...
            let toStoreExercise = {} , toStoreDetail = {};
            toStoreExercise[exercise_id]=exercise;
            toStoreDetail[exercise_detail_id]=exercise_detail || {} ;

            // if(editable){
              const responseOnUpdate = await updateExercise({exercise_id,data:updateData});
              const {status,statusCode,payload:{data:resp_data={},message:resp_msg=''} ={} } = responseOnUpdate;
              if(status){

                const { exercise_details = {} , exercise_detail_id = null } = resp_data || {};

                if( exercise_detail_id ){
                  editedExerciseGroupData[`exercise_detail_id`]=parseInt(exercise_detail_id);
