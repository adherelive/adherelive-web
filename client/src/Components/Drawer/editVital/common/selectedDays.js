...
class SelectedDays extends Component {
    ...
    render() {
        ...
        const {selectedDays} = this.state;
        const {vitalData = {}, payload: {canViewDetails = false} = {}} = this.props;
        let {repeat_days: existing_repeat_days = ''} = vitalData || {};
        if (!existing_repeat_days) {
            const {details: {repeat_days: vital_repeat_days_new = []} = {}} = vitalData || {};
            existing_repeat_days = vital_repeat_days_new;
        ...
        return (
            <div className="select-days-form-content">
                ...
                >
                    <RadioButton value={1}
                                 onClick={this.setRepeatEveryDay}>{this.formatMessage(messages.everyday)}</RadioButton>
                    <RadioButton value={2}
                                 onClick={this.setRepeatAlternateDay}>{this.formatMessage(messages.alternate)}</RadioButton>
                </RadioGroup>
