...
class MissedMedicationsDrawer extends Component {
      ...
      const {
        ...
        payload: {
          data: {
            ...
            critical_medication_event_ids = [],
            non_critical_medication_event_ids = []
          }
        } = {},
        statusCode
      } = response || {};
  ...
  getMedicationList = () => {
      ...
      return (
        <div className=" br5 bg-white flex-shrink-0 mt20 mb20 p10  ml10 mr10 chart-box-shadow  flex direction-column">
          <div className="flex direction-row align-start mt10 mb10">
            ...
            <div
              className=" pointer fs14 fw700 black-85 ml20 tab-color"
              onClick={() => this.handlePatientDetailsRedirect(pId)}
            >
              {pName}
            </div>
          </div>
          ...
          <div className="flex direction-row align-start mt10 mb10">
            <div className="fs14 fw700 brown-grey">
              {this.formatMessage(messages.medicine_time)}
            </div>
          ...
          <div className="flex direction-row align-start mt10 mb10">
            <div className="fs14 fw700 brown-grey">
              {this.formatMessage(messages.medicine_name)}
            </div>
    ...
    for (let medication in missed_medications) {
      const {
        medications: {
          details: { start_time, end_time, medicine_id, repeat_days } = {},
          participant_id = ""
        } = {},
        medicines: {
          basic_info: {
      ...
      let time = start_time ? td.utc().format("hh:mm A") : "--";

      let isCritical =
        criticalMedicationIds.includes(event_id) &&
    ...
    return (
      <Fragment>
        <Drawer
          ...
          width={`30%`}
        >
          <div className="mt20 black-85 wp100">{this.getMedicationList()}</div>
        </Drawer>
      </Fragment>
    );
  }
