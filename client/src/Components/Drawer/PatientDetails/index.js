...
import {
    GENDER, PATIENT_BOX_CONTENT, MISSED_MEDICATION, MISSED_ACTIONS, DIAGNOSIS_TYPE,
...
class PatientDetailsDrawer extends Component {
        ...
        const medicationList = carePlanMedicationIds.map(id => {
            ...
            return (
                <div key={id} className="flex justify-space-between align-center mb10">
                    <div
                        className="pointer tab-color fw600 wp35 tooltip">{name.length > 20 ? name.substring(0, 21) + '...' : name}
    ...
    getAppointmentList = () => {
        ...
        const appointmentList = appointmentsListIds.map(id => {
            ...
            return (
                <div key={id} className="flex justify-space-between align-center mb10">
                    <div
                        className="pointer tab-color fw600 wp35 tooltip">{type_description.length > 0 ? type_description : " "}
    ...
    getPatientDetailContent = () => {
        const {
            auth = {}, treatments = {}, doctors = {}, conditions = {},
            severity: severities = {},
            providers,
            patients,
            payload,
                ...
                <Fragment>
                    ...
                    <div className="wp100 flex justify-space-between align-center mt20">
                        <div className="flex justify-space-around align-center">
                            <div
                                className="pr10 fs24 fw600">{`${getName(first_name)}  ${getName(middle_name)} ${getName(last_name)}`}</div>
                            <div
                                className="pr10 fs20 fw500">{`(${gender ? `${GENDER[gender].view} ` : ''}${age ? age : '--'})`}</div>
                    ...
                    <div
                        className="fw700 wp100">{`${formatMessage(messages.mobile)}: +${prefix} ${mobile_number}`}</div>
                    ...
                    <div className="mt20 wp100">
                        ...
                        <div className="fw500 black-85">
                            ...
                            <div className="flex justify-space-between align-center">
                                ...
                                <div
                                    className="flex-2">{doctor_first_name ? `${doctor_first_name} ${doctor_middle_name ? `${doctor_middle_name} ` : ""}${doctor_last_name ? `${doctor_last_name} ` : ""}` : "--"}</div>
                            </div>
                            <div className="flex justify-space-between align-center">
                            ...
                            <div className="flex justify-space-between align-center">
                                ...
                                <div
                                    className="flex-2">{description ? `${description} (${diagnosis})` : `${"--"} (${diagnosis})`}</div>
                            </div>
                            <div className="flex justify-space-between align-center">
                            ...
                            <div className="flex justify-space-between align-center">
                                ...
                                <div
                                    className="flex-2">{start_date ? moment(start_date).format("Do MMM YYYY") : '--'}</div>
                            </div>
                            <div className="flex justify-space-between align-center">
                                <div className="flex-1">{formatMessage(messages.provider)}</div>
