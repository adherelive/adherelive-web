              ...
              ${similar_index === null ? "null" : "similar-before-line"}
               `}>
                ...
                <div className="fs14 fw700 flex">
                    {`${serving}x${" "}${portion_size}${" "}${portion}`}<span
                    className="grey-dot">&bull;</span>{`${calVal ? calVal : "--"} Cal`}
                </div>
                {
                    notes
                    &&
                    <div
                        className="fs12 ellipsis wp100 max-h-50">{`${this.formatMessage(messages.note)}: `}{notes}</div>
                }
            </div>
