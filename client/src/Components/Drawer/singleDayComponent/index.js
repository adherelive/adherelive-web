...
class DayDiet extends Component{
    ...
    getServingWiseCalorificValue = ({food_item_detail_id,serving : serv}) => {
      ...
      const { basic_info : { calorific_value = 0 } = {} } = food_item_details[food_item_detail_id] || {};
      calVal =  calorific_value*serving;
      return calVal;
    }

    addFoodGroup = async (data) => { // Add Food Group onSubmit
      ...
      const calVal = this.getServingWiseCalorificValue({ food_item_detail_id,serving });
    ...
    editFoodGroup = async(foodGroupDrawerData) => {
      ...
      const { selectedTime } = this.state;
       let { setFinalDayData , completeData , total_calories = 0 , setNewTotalCal } = this.props;
       let newCompleteData = completeData;
    ...
    editSimilarFoodGroup  = async(foodGroupDrawerData) => {
      ...
      const { food_group_index , editedFoodGroupData , similar_index = null  } = foodGroupDrawerData;
       ...
       for(let each of ALL_TIMINGS){
         ...
         if(singleTimeData.length){
           ...
           for(let i = 0;i<singleTimeData.length;i++){
             ...
             const foodGroupComponent = 
              this.getFoodItemComponent({
                food_group_index,
                food_item_detail_id,
                portion_id,
                serving,
    ...
    handleOpenEditFoodGroupDrawer = (data) => () =>  {
      ...
      if(food_group_id){ // else no food group id key 
    ...
    handleDeleteFoodGroup =  ({food_group_index,time}) => async () => {
      const { completeData , setDeletedFoodGroupId, food_item_details , total_calories = 0 , setNewTotalCal , setFinalDayData  } = this.props;
    ...
    handleDeleteSimilarFoodGroup =  ({food_group_index,time,similar_index}) => async () => {
      const { completeData , food_item_details , total_calories = 0 , setNewTotalCal , setFinalDayData ,setDeletedFoodGroupId } = this.props;
      ...
      const { 
        food_item_detail_id = null,
        serving = 1,
        food_group_id = null } = similarFoodGroup; 
     ...
     getFoodItemComponent = ({
        ...
        return (
              ...
              <div className="fs14 fw700 flex" >
                {`${serving}x${" "}${portion_size}${" "}${portion}`}<span className="grey-dot" >&bull;</span>{`${calVal ? calVal : "--"} Cal`}
              </div>
                {
                  notes 
                  &&
                  <div className="fs12 ellipsis wp100 max-h-50" >{`${this.formatMessage(messages.note)}: `}{notes}</div>
                }
            </div>
    ...
    render() {
      const {  
        ...
        editFoodGroupDetails ,
        selectedFoodGroupIndex = null } = this.state;
      ...
      const {
        ...
        closeAddSimilarFoodGroup,
        addFoodGroup ,
        submitAddSimilarFoodGroup } = this;
