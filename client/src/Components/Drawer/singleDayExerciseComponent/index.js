...
class DayWorkout extends Component{
    ...
    addExerciseGroup = async (data) => { // Add Exercise Group onSubmit
    ...
    editExerciseGroup = async(exerciseGroupDrawerData) => {
      ...
      const { exercise_group_index , editedExerciseGroupData  } = exerciseGroupDrawerData;
    ...
    getTimingOptions = () => {
      ...
      for(let i = 0;i<completeData.length;i++){
        ...
        const {
          ...
          const { basic_info : { repetition_id = null } = {} } = exercise_details[exercise_detail_id] || {};

        const exercise_group_index = i;

        const exerciseGroupComponent = 
        this.getExerciseItemComponent({
    ...
    handleOpenEditExerciseGroupDrawer = (data) => () =>  {
      ...
      if(exercise_group_id){ // else no exercise group id key 
    ...
    handleDeleteExerciseGroup =  ({exercise_group_index}) => async () => {
      const { completeData , setDeletedExerciseGroupId, exercise_details , total_calories = 0 , setNewTotalCal , setFinalDayData  } = this.props;
     ...
     getExerciseItemComponent = ({
        ...
        return (
                  ...
                  <div className="flex direction-column algin-center justify-center" >
                    {
                      ...
                      <img
                        ...
                        onClick={
                          handleOpenEditExerciseGroupDrawer({
                            exercise_group_index,
                            sets,
              ...
              <div className="fs14 fw700 flex" >
                {`${sets}x${" "}${repetition_value}${" "}${repetition}`}<span className="grey-dot" >&bull;</span>{`${calVal ? calVal : "--"} Cal`}
              </div>
                {
                  notes 
                  &&
    ...
    render() {
      const {  
        ...
        editExerciseGroupDetails ,
        selectedExerciseGroupIndex = null } = this.state;
