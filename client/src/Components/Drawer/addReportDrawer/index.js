...
class addReportDrawer extends Component{
  ...
  getUploadedDocuments = () => {
    ...
    return documents.map((each,index)=> {
      ...
      const {name='', src} =each;
      const documentExtension = name.substring(
          name.length - 3,
          name.length
    );
    ...
    try{
      ...
      const { 
        ...
        for(let each of documents){
          ...
          const {
            status = false,
            payload: { message: respMessage = "" , data :{documents :response_documents = [] }={} } = {}
          } = response;
  ...
  handleCloseWarning = () => {
    ...
    render(){
      ...
      const disabledSubmit = (!name || Object.keys(documents).length===0 || !test_date) ; 
