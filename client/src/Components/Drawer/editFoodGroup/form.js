...
class EditFoodGroupForm extends Component {
  ...
  async componentDidUpdate(prevProps){
    ...
    const {basic_info : { food_item_id : previous_item_id = null } = {} } = searched_food_item_details[food_item_detail_id] || {};
    const {basic_info : {name = ''} = {} } = searched_food_items[previous_item_id] || {};


    const {
      food_item_detail_id : prev_food_item_detail_id = null,
      visibleAddFoodDrawer : prev_visibleAddFoodDrawer = false  } = prevProps;
    ...
    if(!visibleAddFoodDrawer && prev_visibleAddFoodDrawer !== visibleAddFoodDrawer){
      if(created){  // fill new created food's details
  ...
  fillAllNewCreatedFoodDetails = () => {
      ...
      const { latest_created_food : { food_items = {} , food_item_details = {} } = {} } = this.props;
      const { setFoodItemName , setEditable , setFoodItemDetailId } = this.props;
      let food_item_id = null , detail_id = null;
  ...
  getPortionOptions = () => {
    ...
    const {food_item_id : state_food_item_id = null , doctor_id = null }  = this.state;
    const { searched_food_item_details ,
      portions = {} ,
    ...
    for(let each in searched_food_item_details ){
        ...
        const {
         ...
         if( state_food_item_id == food_item_id ){
            ...
            options.push(
                <Option
                 key={`${each}-${name}`} value={portion_id}
  ...
  handleExistingPortionSelect = ({detail_id:value,editable}) => () => {
    ...
    handleDifferentPortionSelect = ({editable}) => () => { // portion doesnt exist for item
  ...
  getFoodOptions = () => {
    ...
    for(let each in searched_foods ){
        ...
        const { basic_info : { name : food_name ,  id : food_item_id } = {} } = food || {};
  ...
  setFoodItemId = (value) =>  {
    ...
    const { doctor_id = null , food_item_id : prev_food_item_id = null } = this.state;
    let first = null;
    ...
    if( prev_food_item_id !== value && Object.keys(searched_food_item_details).length){
      let editable = false;

      for(let each in searched_food_item_details ){
        const {basic_info : { food_item_id , creator_type = null , creator_id = null } = {} } = searched_food_item_details[each] || {};
  ...
  render() {
    const {
      ...
      editFoodGroupDetails , food_item_details,
      editable=false
    } = this.props;
    ...
    const {
      ...
      notes = null } = editFoodGroupDetails || {};
        ...
        <div className="wp100 flex algin-center justify-space-between" >
          ...
          {/* proteins */}           
            ...
            {/* carbs */}
              ...
              {/* fats */}
              <FormItem
                label={formatMessage(messages.fats)}
                className=" mt-4 wp30"
              >
