...
class EditFoodGroup extends Component {
  ...
  handleSubmit = (e) => {
    ...
    const { food_item_detail_id = null,editable = false , food_item_name = ''  } = this.state;

    const { detail_id : prev_detail_id = null , food_group_index = null , similar_index = null , food_group_id = null , serving : prev_serving = 1 } = editFoodGroupDetails || {};
    ...
    validateFields(async (err, values) => {
      if (!err) {
        ...
        const food_item_detail = searched_food_item_details[food_item_detail_id];
        const food_item = searched_food_items[food_item_id] || {} ;
        let prev_calorific_val = 0;
        const { basic_info : { calorific_value :prev_calVal = 0 } = {} } = food_item_details[prev_detail_id] || {};
        ...
        let editedFoodGroupData = {
          ...
          food_item_detail_id: food_item_detail_id ? parseInt(food_item_detail_id) : null,
          notes,
          prev_calorific_val,
          prev_serving,
        ...
        const updateData={
          ...
          try {
            ...
            let toStoreFoodItem = {} , toStoreDetail = {};
            toStoreFoodItem[food_item_id]=food_item;
            toStoreDetail[food_item_detail_id]=food_item_detail || {} ;

            if(editable){
              const responseOnUpdate = await updateFoodItem({food_item_id,data:updateData});
