...
import { SORT_CREATEDAT,
    SORT_NAME,
    FILTER_DIAGNOSIS,
    FILTER_TREATMENT,
    OFFSET } from "../../Dashboard/index";
...
class patientTable extends Component{
    ...
    handleSearchTreatmentGetPatient = async (flag = false) => {
        try{
            ...
            const {status,payload : { data:{total}={}, message :resp_message = ''} = {}} = response;
            this.setState({total});
            if(!status){
    ...
    getDataSource = () => {
        ...
        const {
             searchTreatmentText='' , searchDiagnosisText ='' }=this.state;
  ...
  onChange = (pagination, filters, sorter, extra) => {
    ...
    const {currentTab = CURRENT_TAB.ALL_PATIENTS ,tabState={} , sortByName , sortByCreatedAt ,
    changeTabState } =this.props;

    const {searchTreatmentText = '' , searchDiagnosisText = ''}=this.state;

    if((columnKey !== TABLE_COLUMN.CREATED_AT.key && columnKey !== TABLE_COLUMN.PID.key)
        ||
