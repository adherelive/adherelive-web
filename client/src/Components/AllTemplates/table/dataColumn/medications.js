...
import { MEDICATION_TIMING,DAYS_TEXT_NUM_SHORT, EVENT_TYPE, MEDICATION_TIMING_HOURS, MEDICATION_TIMING_MINUTES, TABLET, SYRUP, MEDICINE_FORM_TYPE } from "../../../../constant";
...
export default props => {
    const {template_medication_ids , medicationsData = {} , medicines = {}} = props;
//   const { medicationsData : { basic_info: { medicine_id  = '' } = {} } = {}  , template_medication_ids } = props || {};
  ...
  for(let key of template_medication_ids){
    ...
    let {basic_info : {medicine_id  = '' } = {} , schedule_data: { when_to_take = '', start_date = moment(), medicine_type = '1' ,repeat_days=[]} = {} } = medicationsData[key];
            ...
            meds.push(
                    <div className='flex wp100 align-center ' key={key}>
                        <div className='flex direction-column mb10' >
                            ...
                            < div className='flex direction-column'>{medTimingsToShow}{more > 0 ? `+${more} more` : ''}</div>
