...
export default props => {
  ...
  for(let key of template_appointment_ids){
    ...
    const { reason = '', details: { description = '', date = '', start_time = '' } = {}, time_gap = '' } = appointmentsData[key] || {} ;
    ...
    appointments.push(
        <div className='flex wp100 align-center ' key={key}>
            <div className='flex direction-column mb10' >
                ...
                < div className='flex direction-column'>{date ? `After ${moment(date).diff(moment(), 'days')} days` : time_gap ? `After ${time_gap - 1} days` : ''}</div>
