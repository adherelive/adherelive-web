...
class favouriteMedicine extends Component{
    ...
    getFavouriteOptions = () => {
        ...
        const {favourites_data , favourite_medicine_ids,onFavOptionSelect ,getFavouriteSelectedMedicine } = this.props;
        const options = [];

        console.log("98346753264792834657234672930 ########################",{L:Object.keys(favourites_data).length,favourites_data});
        for(let each in favourites_data ){
            ...
            if(marked_favourite_type === "medicine"){
                ...
                options.push(
                    <li key={`opt-${medicine_id}`} value={medicine_id}
                    onMouseDown={this.handleOnMouseDownPreventDef}
                    // onClick={onFavOptionSelect(medicine_id)}
                    onClick={this.handleGetFavouriteSelectedMedicine(medicine_id,medicine_name)}
                    >
    ...
    render(){
        ...
        console.log("9836462746239846239 searching_medicine",{searching_medicine});
        return (
            <div className={`fav-list ${medicineIdSelected && searching_medicine ? "mt80-I" : "" }`} >
