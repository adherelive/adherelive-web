...
export default props => {
    ...
    confirm({
      ...
      onOk: async() => {
        ...
        try{
          ...
          const {makeMedicinePublic , getPrivateMedicines , searchText=''} =props;
          changeLoading(true);
          const response = await makeMedicinePublic({medicine_id,offset});
          const {payload : {data : { medicines = {} } ={}, message:resp_message} = {} ,status}=response;
          let medicine = {};
          const medicineKey = Object.keys(medicines)[0];
          const {basic_info = {}} = Object.values(medicines)[0];

          const medicineValue = {basic_info,["updated_at"]:moment().toISOString()};
  ...
  return (
    ...
    <div 
    className="wp100 flex justify-end align-center"
    >
      <Tooltip title={public_medicine ? formatMessage(messages.public) : formatMessage(messages.makePublic)} > 
          <Icon
          className="pointer align-self-end mr20 fs20 align-self-end"
          onClick={public_medicine ? '' : handleMakeMedicinePublic}
          type="check-circle" theme="twoTone" twoToneColor={public_medicine ? "#52c41a" : "pink"}
          />
        </Tooltip>
