...
class MedicineTable extends Component {
  ...
  componentDidUpdate(prevProps,prevState){
    console.log("782657387923785623789233 CD UPDATEEEEEEEEEEEEEEEEEeeeeeeeeeeeeeee");
    ...
    if(searchPrivateText !== '' && searchPrivateText !== prev_searchPrivateText){
  ...
  async handleSearchPublicMedicine(value,offset){
    try{
      ...
      console.log("3275437847237845324625347879198371293712945216",{response,value,offset});
      const {payload : {data : {total_count = 0} = {} , response:resp_msg='' } = {} } =response || {};
  ...
  formatMessage = (message, ...rest) => this.props.intl.formatMessage(message, rest);
  ...
  getDataSource = () => {
    ...
    console.log("98327548237469238048230490 getDataaaaaaaaaaaaaa ",{props_public_medicines,props_private_medicines});
      if (key >= 0) {
        if (currentTab === ALL_TABS.PUBLIC) {
          if (props_public_medicines[key] && searchPublicText === '' ) {
            public_medicines = props_public_medicines[key];
          }else if(props_search_public_medicines[key] && searchPublicText !== ''){
          ...
          }else if (props_search_private_medicines[key]  &&  searchPrivateText !== ''){
      ...
      return (
        <div style={{ padding: 8 }}>
          <Input
            ...
            placeholder={`Search ${this.getPlaceholderSearch(data.dataIndex)}`}
  ...
  render() {
    ...
    const {loading = true , searchPublicText='' , searchPrivateText='',searchPublicCount=null , searchPrivateCount=null } = this.state;
      ...
      <Table
        ...
        pagination={{
          ...
          pageSize: currentPage === 1 
           ? Object.keys(obj).length ? Object.keys(obj).length : parseInt(pageSize)
           :  parseInt(pageSize),
          total: currentTab === ALL_TABS.PUBLIC
