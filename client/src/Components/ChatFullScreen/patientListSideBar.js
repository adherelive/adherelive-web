...
const Header = ({handleSearch, formatMessage}) => {
    return (
        <div className='chat-patientListheader'>
            <Input className='patientSearch' placeholder={formatMessage(messages.searchPatient)}
...
class PatientListSideBar extends Component {
    ...
    renderPatients = () => {
        ...
        let allPatients = Object.values(searchPatientIds).map((patient) => {
            const {
                basic_info: {id = 0, first_name = '', middle_name = '', last_name = ''} = {},
                details: {profile_pic: patientDp = ''} = {}
            } = patients[patient];
            return (
                <div key={id}
                     className={parseInt(id) === parseInt(patientId) ? 'chat-patient-card-parent-selected' : 'chat-patient-card-parent'}>
                    <PatientCard setPatientId={setPatientId} patientId={id}
