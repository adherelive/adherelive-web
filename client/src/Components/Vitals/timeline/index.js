...
class VitalTimeline extends Component {
    ...
    handleEditVitalResponse = (data) => async (e) => {
        ...
        try {
            ...
            const {
                status,
                payload: {data: {vital_date_ids, vital_timeline} = {}, message: responseMessage = ""} = {}
            } = response;
            if (status === true) {
                                ...
                                <Fragment>
                                    ...
                                    >
                                        ...
                                        })}
                                        {
                                            ...
                                            (
                                                <div
                                                    className="edit-delete-vitals flex align-center"> {/*edit-delete-vitals*/}
                                                    {/*  EDIT  */}
                                                    {/*<div onClick={openEditVitalResponseModal({response: res,event, id, index: responseIndex})}>*/}
                                                    <Button type={"secondary"} onClick={openEditVitalResponseModal({
                                                        response: res,
                                                        event,
                                                        id,
                                                        index: responseIndex
                                                    })}>{this.formatMessage(messages.edit_response_btn)}</Button>
                                                    {/*</div>*/}

                                                    {/*  DELETE  */}
                                                    {/*<div onClick={handleDeleteVitalResponse({id, index: responseIndex})}>*/}
                                                    <Button type={"danger"} ghost className="ml10 fs14 no-border"
                                                            onClick={handleDeleteVitalResponse({
                                                                id,
                                                                index: responseIndex
                                                            })}>{this.formatMessage(messages.delete_response_btn)}</Button>
                                                    {/*</div>*/}
                                                </div>
                                            )
                                        }
