...
class TosPPEditorPage extends Component {
    ...
    handleSubmit = async (e) => {
        ...
        try {
            const response = await updateTermsAndPolicy({type: feature_type, content: value});
    ...
    getHeader = () => {
        const {handleSubmit, formatMessage, handleOptionChange, handleBack} = this;
        return (
            <Fragment>
                <div className="wp100 mb20 fs28 fw700 flex align-start justify-space-between">
                    <div className="flex flex-start align-center mb20">
                        ...
                        <div className="fs30 fw700">{formatMessage(messages.details_page_header)}</div>
                    </div>
                    <Button type="primary" onClick={handleSubmit}>{formatMessage(messages.save_text)}</Button>
                </div>

                <RadioGroup onChange={handleOptionChange} defaultValue={TERMS_OF_SERVICE} buttonStyle={"solid"} className="mb20">
                    <RadioButton value={TERMS_OF_SERVICE}>{formatMessage(messages.terms_of_service_text)}</RadioButton>
                    <RadioButton value={PRIVACY_POLICY}>{formatMessage(messages.privacy_policy_text)}</RadioButton>
                </RadioGroup>
            </Fragment>
