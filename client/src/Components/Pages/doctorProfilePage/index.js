...
class DoctorProfilePage extends Component {
        ...
        try {
            ...
            if(status){
                ...
                if(statusCode == HTTP_CODE_SERVER_ERROR){
                    const { payload: {message:{message:serverMessage}} } = response;
    ...
    updateName = () => {
      ...
      const oldName= `${first_name} ${middle_name ? `${middle_name} ` : ""}${last_name ? last_name : ""}`;
    ...
    updateRegistrationNumber = (e) => {
      ...
      if(e.target.value==''){
        ...
        this.setState({edit_registration_number:newEditRegistrationNumber,registration_numbers:newRegistrationNumber});
    ...
    updateClinicName = (e) => {
      ...
      const { basic_info: { name : oldName } } = doctor_clinics[clinic_id]
      let updateData = {
          clinic_details : [
              {
                  name:e.target.value,
      ...
      if(e.target.value==''){
        ...
        this.setState({edit_clinic_name:newEditClinic,clinic_names:newClinicName});
    ...
    updateClinicLocation = (e) => {
        const { doctor_user_id, edit_clinic_location, doctor_clinic_location } = this.state;
        const { doctor_clinics } =this.props;
        ...
        if(e.target.value == ''){
          ...
          this.setState({edit_clinic_location:newEditClinicLocation,doctor_clinic_location:newClinicLocation}); 
  ...
  renderName = () => {
      ...
      if(edit_name){
        return (
            <div style={{position:"relative"}} >
                <Input autoFocus onBlur={this.updateName} value={name} onPressEnter={this.updateName} onChange={this.onChangeName}></Input>
                <div style={{position:"absolute",right:0}}>
                    <CheckOutlined className={"update-button-ok"}/>
                    <CloseOutlined className={"update-button-cancel"} onMouseDown={this.undoNameChanges}/>
                </div>
            </div>
        );
      }else{
    ...
    renderRegistrationNumber = (registration_id) => {
        const { edit_registration_number, verified_doctor, doctor_user_id,registration_numbers } =this.state;
        ...
        return (
            <div style={{position:"relative"}}>
              ...
              <div style={{position:"absolute",right:0}}>
                <CheckOutlined className={"update-button-ok"}/>
                <CloseOutlined className={"update-button-cancel"} onMouseDown={()=>this.undoRegistrationNumberChanges(registration_id)}/>
              </div>
            </div>
        );
        }else{
    ...
    renderClinicName = (clinic_id) => {
        ...
        return (
            <div style={{position:"relative"}}>
              <Input value={clinicName} onChange={this.onChangeClinicName} onPressEnter={this.updateClinicName} data-id={clinic_id} onBlur={this.updateClinicName} autoFocus></Input>
              <div style={{position:"absolute",right:0}}>
                  <CheckOutlined className={"update-button-ok"}/>
                  <CloseOutlined className={"update-button-cancel"} onMouseDown={()=>this.undoClinicNameChanges(clinic_id)}/>
              </div>
            </div>
        );
        }else{
                    ...
                    {({ getInputProps, suggestions, getSuggestionItemProps, loading }) => (
                        <div>
                            <Input
                                ...
                                autoFocus
                            />
                            <div className="google-places-autocomplete__suggestions-container" style={{position:"absolute",backgroundColor:"white",marginTop:"4px"}}>
                            ...
                            {!loading && suggestions.length==0 && <div style={{float:"right"}}>
                              <CheckOutlined className={"update-button-ok"}/>
                              <CloseOutlined className={"update-button-cancel"} onMouseDown={()=>this.undoLocationChanges(clinic_id)}/>
                            </div>}
                        </div>
                    )}
                </PlacesAutocomplete>
    ...
    renderQualificationYear = (qualification_id) => {
        ...
        if(edit_qualification_year[qualification_id]){
          return (
                <Select onBlur={()=> this.onBlurQualificationYear(qualification_id)} size={"small"} style={{width:"100px",height:"auto",margin:"0"}} className="form-inputs" placeholder="Select Year" value={doctor_qualifications[qualification_id].basic_info.year ? doctor_qualifications[qualification_id].basic_info.year : null} onChange={this.updateQualificationYear(qualification_id)} autoFocus>
                    {this.getYearOptions()}
                </Select>
          );
        }else{
    ...
    renderRegistrationYear = (registration_id) => {
        ...
        if(edit_registration_year[registration_id]){
          return (
                <Select size={"small"} style={{width:"100px",height:"auto",margin:"0"}} className="form-inputs" placeholder="Select Year" value={doctor_registrations[registration_id].basic_info.year ? doctor_registrations[registration_id].basic_info.year : null} onChange={this.updateRegistrationYear(registration_id)} onBlur={()=> this.onBlurRegistrationYear(registration_id)} autoFocus>
                    {this.getYearOptions()}
                </Select>
          );
        }else{
    ...
    renderRegistrationExpiryDate = (registration_id) => {
        ...
        if(edit_registration_expiry[registration_id]){
          return (
            <DatePicker  style={{margin:"0",height:"auto"}} size={"small"} defaultValue={moment(doctor_registrations[registration_id].expiry_date)} onChange={this.updateRegistrationExpiryDate(registration_id)} placeholder='Select Expiry Date' open={true} autoFocus/>
          );
        }else{
              return (
                  <>
              ...
              >
                  {({ getInputProps, suggestions, getSuggestionItemProps, loading }) => (
                      <div>
                          ...
                          {!loading && suggestions.length==0 && <div style={{float:"right"}}>
                            <CheckOutlined className={"update-button-ok"}/>
                            <CloseOutlined className={"update-button-cancel"} onMouseDown={this.undoCityChanges}/>
                          </div>}
                      </div>
                  )}
              </PlacesAutocomplete>
