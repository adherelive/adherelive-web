...
class doctorCalender extends Component {
  ...
  componentDidMount() {
    ...
    for (let i = 0; i < 12; i++) {
      let ISOdate = moment(startOfYear)
        .add(i, "M")
        .toISOString();
  ...
  getListData = (appointmentIds = []) => {
    ...
    const { doctors = {}, patients = {} } = selectedMonthAppointments || {};
    let count = 0;
  ...
  getMonthData = value => {
    ...
    for (let key in date_wise_appointments) {
      ...
      if (
        keyDate.year() === value.year() &&
        keyDate.month() === value.month()
      ) {
      ...
      details.push(
        ...
        >
            <div className="absolute r10">
                {critical ?
                    (
                        <Tooltip align={"top"} title={this.formatMessage(messages.critical)}>
